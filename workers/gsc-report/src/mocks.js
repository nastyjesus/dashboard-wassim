...
// à de la vraie data. Générateur déterministe (seed = propriété + période) pour
// que les tests et les démos soient reproductibles.
...
/**
 * Série quotidienne + totaux mock pour une période donnée.
 * @param {string} property
 * @param {string} startDate YYYY-MM-DD
 * @param {string} endDate YYYY-MM-DD
 */
export function mockPeriodData(property, startDate, endDate) {
  const rand = rng(`${property}|${startDate}`);
  ...
  for (let i = 0; i < days; i++) {
    ...
    const clicks = Math.max(0, Math.round((base + (rand() - 0.35) * base) * weekendDip));
    const impressions = clicks * (14 + Math.round(rand() * 18));
    series.push({ date: d.toISOString().slice(0, 10), clicks, impressions });
  }
  const clicks = series.reduce((s, d) => s + d.clicks, 0);
  const impressions = series.reduce((s, d) => s + d.impressions, 0);
  const ctr = impressions ? clicks / impressions : 0;
  const position = 8 + rand() * 22;

  const queries = MOCK_QUERIES.map((q, i) => ({
    keys: [q],
    clicks: Math.max(1, Math.round(clicks * (0.18 - i * 0.015) * (0.7 + rand() * 0.6))),
    impressions: Math.max(10, Math.round(impressions * (0.12 - i * 0.008))),
    ctr: 0.01 + rand() * 0.08,
    position: 3 + rand() * 30,
  ...
  const pages = MOCK_PAGES.map((p, i) => ({
    ...
    clicks: Math.max(1, Math.round(clicks * (0.2 - i * 0.016) * (0.7 + rand() * 0.6))),
    impressions: Math.max(10, Math.round(impressions * (0.13 - i * 0.009))),
    ctr: 0.01 + rand() * 0.08,
    position: 3 + rand() * 30,
  })).sort((a, b) => b.clicks - a.clicks);
